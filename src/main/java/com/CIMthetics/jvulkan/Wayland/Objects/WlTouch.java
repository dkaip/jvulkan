...
public class WlTouch extends WaylandInterfaceObject
{
    private Logger log = LoggerFactory.getLogger(WlTouch.class.getName());
    private WlDisplaySingleton wlDisplay = WlDisplaySingleton.getInstance();

    public WlTouch(WlTouchHandle handle)
    {
        setHandle(handle);
    ...
    @Override
    WlTouchHandle getHandle()
    {
        return (WlTouchHandle)handle;
    }
